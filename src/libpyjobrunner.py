...
from pythonjobrunner_cfg import CFG
...
class cBioJob(Job):
    ...
    def is_out_of_memory(self):
        ...
        if self.heart_beat != None:
            ...
            unit = self.mem[-2:]
            allocated_mb = float(self.mem[0:-2])
    ...
    def alter_allocated_memory(self, factor=2.0):
        ...
        if self.mem != "":
            unit = self.mem[-1]
            allocated_mb = float(self.mem[0:-1])
            self.mem = "%f%s" % (factor*allocated_mb, unit)

            print "memory for job %s increased to %s" % (self.name, self.mem)
...
def handle_resubmit(session_id, job):
    ...
    job.mem will be doubled, if cause was 'out_of_memory'
    """
...
def send_error_mail(job):
    ...
    body_text += "requested memory: " + str(job.mem) + "\n"
